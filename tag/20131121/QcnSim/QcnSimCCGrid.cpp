 ...
 */
std::shared_ptr<SimplexLink> findLinkByNodeB(std::map<unsigned int, std::shared_ptr<SimplexLink>> linkMap, std::shared_ptr<Node> nodeB) {
...
int main(int argc, char *argv[]) {
	...
	std::map<unsigned int, std::shared_ptr<SimplexLink>> linkMap;
	std::map<unsigned int, std::vector<std::shared_ptr<Entity>>> explicitRouteMap;
	std::shared_ptr<SimplexLink> link(nullptr);
	...
	//linkMap.insert(std::pair<unsigned int, std::shared_ptr<SimplexLink>>(REGION_A_DESTINATION, std::make_shared<SimplexLink>(SimplexLink(nodeMap.at(REGION_A_SOURCE), nodeMap.at(REGION_A_DESTINATION),
	//	BANDWIDTH_REGION_A, PROPAGATION_REGION_A, simulatorGlobals, scheduler, "link region A"))));
	//linkMap.insert(std::pair<unsigned int, std::shared_ptr<SimplexLink>>(REGION_B_DESTINATION, std::make_shared<SimplexLink>(SimplexLink(nodeMap.at(REGION_B_SOURCE), nodeMap.at(REGION_B_DESTINATION),
	//	BANDWIDTH_REGION_B, PROPAGATION_REGION_B, simulatorGlobals, scheduler, "link region B"))));
	//linkMap.insert(std::pair<unsigned int, std::shared_ptr<SimplexLink>>(REGION_C_DESTINATION, std::make_shared<SimplexLink>(SimplexLink(nodeMap.at(REGION_C_SOURCE), nodeMap.at(REGION_C_DESTINATION),
	//	BANDWIDTH_REGION_C, PROPAGATION_REGION_C, simulatorGlobals, scheduler, "link region C and D"))));
	////linkMap.insert(std::pair<unsigned int, std::shared_ptr<SimplexLink>>(REGION_D_DESTINATION, std::make_shared<SimplexLink>(SimplexLink(nodeMap.at(REGION_D_SOURCE), nodeMap.at(REGION_D_DESTINATION),
	////	BANDWIDTH_REGION_D, PROPAGATION_REGION_D, simulatorGlobals, scheduler, "link region D"))));
	//linkMap.insert(std::pair<unsigned int, std::shared_ptr<SimplexLink>>(REGION_D_DESTINATION, linkMap.at(REGION_C_DESTINATION))); // Same link as Region C.
	...
	linkMap.insert(std::pair<unsigned int, std::shared_ptr<SimplexLink>>(REGION_B_DESTINATION, linkMap.at(REGION_A_DESTINATION))); 
	linkMap.insert(std::pair<unsigned int, std::shared_ptr<SimplexLink>>(REGION_C_DESTINATION, linkMap.at(REGION_A_DESTINATION))); 
	linkMap.insert(std::pair<unsigned int, std::shared_ptr<SimplexLink>>(REGION_D_DESTINATION, linkMap.at(REGION_A_DESTINATION)));
	linkMap.insert(std::pair<unsigned int, std::shared_ptr<SimplexLink>>(REGION_FAKE_DESTINATION, std::make_shared<SimplexLink>(SimplexLink(nodeMap.at(REGION_FAKE_SOURCE), nodeMap.at(REGION_FAKE_DESTINATION),
	...
	while (!simulationEnded) {
		...
		switch (currentEvent.eventType) {
			...
			case EventType::TRAFFIC_GENERATOR_ARRIVAL:
				...
				// Actually, all operations here can be handled by PDUTOKEN_ARRIVAL_AT_NODE. Just schedule this as next even and pass PDU.
