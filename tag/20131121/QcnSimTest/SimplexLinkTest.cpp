 ...
 */
SimplexLinkTest::SimplexLinkTest(): simulatorGlobals(SimulatorGlobals(0.0, 0.0, false, "LinkTest")), 
		scheduler(Scheduler(simulatorGlobals)) {
}

/// Construct Link, test default values.
TEST_F(SimplexLinkTest, Constructor) {
	...
	SimplexLink linkWithName(source, destination, bandwidth, propagationDelay, simulatorGlobals, scheduler, name);
	SimplexLink linkNoName(source, destination, bandwidth, propagationDelay, simulatorGlobals, scheduler);
...
TEST_F(SimplexLinkTest, TransmitAndPropagate) {
	std::shared_ptr<Node> source(std::make_shared<Node>(simulatorGlobals));
	std::shared_ptr<Node> destination(std::make_shared<Node>(simulatorGlobals));
	...
	// Create simplex link.
	SimplexLink link(source, destination, bandwidth, propagationDelay, simulatorGlobals, scheduler, name);
 ...
 *
 */
TEST_F(SimplexLinkTest, MultiplePdus) {
	// Some variables.
	Event event;
	...
	SimplexLink link01(nodeVector.at(0), nodeVector.at(1), 100000, 0.001, simulatorGlobals, scheduler); // 100 Kbps, 1ms.
	link01.setTransmissionQueueSizeLimit(9); // PDUs above 10 will be discarded.
	SimplexLink link12(nodeVector.at(1), nodeVector.at(2), 100000, 1.0, simulatorGlobals, scheduler); // 100 Kbps, 1s, 3 PDU queue limit.
	link12.setTransmissionQueueSizeLimit(3);
	SimplexLink link13(nodeVector.at(1), nodeVector.at(3), 100000, 10.0, simulatorGlobals, scheduler); // 100 Kbps, 10s, 3 PDU queue limit.
...
*
*/
TEST_F(SimplexLinkTest, MultiplePdusSimpleTopology) {
	// Some variables.
	Event event;
	std::shared_ptr<Entity> eventEntityNonConst; // To temporarily use for removal of constness of Entity within event.
	std::shared_ptr<ProtocolDataUnit> pduFromEventEntityNonConst;
	...
	// Create simplex links between nodes.
	SimplexLink link01(nodeVector.at(0), nodeVector.at(1), 100000000, 0.0001, simulatorGlobals, scheduler); // 100 Mbps, 0.1ms. Very fast.
	//link01.setTransmissionQueueSizeLimit(9); // PDUs above 10 will be discarded.
	SimplexLink link12(nodeVector.at(1), nodeVector.at(2), 10000, 1.0, simulatorGlobals, scheduler); // 10 Kibps, 1s, 3 PDU queue limit. Very slow.
...
*
*/
TEST_F(SimplexLinkTest, MultiplePdusMediumDrop) {
	// Some variables.
	Event event;
	std::shared_ptr<Entity> eventEntityNonConst; // To temporarily use for removal of constness of Entity within event.
	std::shared_ptr<ProtocolDataUnit> pduFromEventEntityNonConst;
	...
	// Create simplex links between nodes.
	SimplexLink link01(nodeVector.at(0), nodeVector.at(1), 8000, 2, simulatorGlobals, scheduler); // 8 Kbps, 10s. Transmission takes 1s per PDU, very slow propagation, to force dropping these PDUs in transit.
