...
void Scheduler::schedule(const Event &event) {
	...
	while (eventIterator != eventChain.end()) {
		if (eventIterator->eventTime <= eventTime) { // Looking for event with eventTime *after* the new event to insert
			++eventIterator;
		} else {
			// Found next event after new event to be inserted. Insert new event before.
			eventChain.insert(eventIterator, EventChainElement(eventTime, event)); // Insert element (new EventChainElement with copy of event).
			return; // Done.
		}
	}
