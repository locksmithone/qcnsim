 ...
 */
Facility::Facility(std::string name, unsigned int numberOfServers, SimulatorGlobals &simulatorGlobals, Scheduler &scheduler): name(name), simulatorGlobals(simulatorGlobals), scheduler(scheduler) {
	this->initializeMembers();
	for (unsigned int i = 0; i < numberOfServers; ++i) {
		servers.push_back(FacilityServer(i));
	}
}
 ...
 * @param scheduler Scheduler object reference, typically to allow for scheduling service a dequeued token.
 */
Facility::Facility(std::string name, SimulatorGlobals &simulatorGlobals, Scheduler &scheduler): name(name), simulatorGlobals(simulatorGlobals), scheduler(scheduler) {
	this->initializeMembers();
	servers.push_back(FacilityServer(0));
}
 ...
 *
 * @todo Verify TARVOS's statistics processing here. 
 *
...
unsigned int Facility::purgeQueue() {
	...
	int dropped = 0; //!< Tokens actually dropped from queue. (This is simply the current queue.size(), but let's count for debugging purposes.)
	while (queue.size() > 0) {
 ...
 *     (but it is not actually dropped in this function). Returns FACILITY_DOWN.
 *
 ...
 *     (that indicates a blocked (not preempted) token). Returns TOKEN_ENQUEUED.
 *
...
FacilityReturnType Facility::request(std::shared_ptr<const Token> token, EventType eventType) {
	std::vector<FacilityServer>::iterator serverIterator;
	...
	if (!isUp_) {
		...
		/// @todo Nothing done with token; where is it deleted?  The server still owns it.
		return FacilityReturnType::FACILITY_DOWN;
	}
	// Get next free server, if any. The return might be an iterator to a free server, or servers.end() if no free server was found.
	serverIterator = nextFreeServer();
	...
	if (serverIterator == servers.end()) {
		...
		return FacilityReturnType::TOKEN_ENQUEUED;
	} else {
		// Put token into service in this server.
		serverIterator->token = token;
		serverIterator->serviceStartTime = simulatorGlobals.getCurrentAbsoluteTime(); // Update start of service.
		serverIterator->isBusy = true; // Mark this server as busy.
 ...
 *      Then, enqueue the token as it is done in the request function. Returns TOKEN_ENQUEUED.
 *
 * @param token Token object for which service is requested.
 * @param eventType Event type of the event that requested service for this token (necessary for enqueing the token, and later scheduling a new request for the dequeued token).
 * @return Return type or result of this token (whether the token was enqueued, put into service, etc.).
 ...
 * @param scheduler Scheduler object, for scheduling a request event for the token at the head of the queue.
 */
FacilityReturnType Facility::release(std::shared_ptr<const Token> token) {
	...
	while (serverIterator != servers.end()) {
		if (serverIterator->isBusy == true && serverIterator->token == token) {
			// Found busy server with the token. Let's release and update statistics.
			serverIterator->isBusy = false;
			serverIterator->token.reset(); // One less reference to the token, i.e., server down not "own" token anymore.
			++serverIterator->releasedTokensCount;
			serverIterator->sumBusyTime += simulatorGlobals.getCurrentAbsoluteTime() - serverIterator->serviceStartTime; // Acumulates busy time for this server.
			// Update facility's statistics.
			this->sumBusyTime += simulatorGlobals.getCurrentAbsoluteTime() - serverIterator->serviceStartTime; // Acumulates busy time for facility.
			++releasedTokensCount;
...
std::vector<FacilityServer>::iterator Facility::nextFreeServer() {
	for (std::vector<FacilityServer>::iterator serverIterator = servers.begin(); serverIterator != servers.end(); ++serverIterator) {
		if (! serverIterator->isBusy) { // Return immediately with reference if one server is found free.
...
std::vector<FacilityServer>::const_iterator Facility::nextFreeServer() const {
	for (std::vector<FacilityServer>::const_iterator serverIterator = servers.begin(); serverIterator != servers.end(); ++serverIterator) {
		if (! serverIterator->isBusy) { // Return immediately with reference if one server is found free.
			return serverIterator;
		}
	}
 ...
 * @return Queue size.
 */
unsigned int Facility::getQueueSize() const {
	return queue.size();
}
 ...
 * Set facility's name.
 */
void Facility::setName(std::string name) {
	this->name = name;
}
 ...
 */
unsigned int Facility::getMaxQueueSize() const {
	return maxQueueSize;
}
