 ...
 * 
 * @par Description
 * The Facility class is used to model a resource or server, i.e., something from which a process requests service.
 * A facility consists of some servers and one priority queue, in which processes await access to the next available server. Only one process at a time can use a server.
...
private:
	...
	unsigned int maxQueueSize;			//!< Maximum recorded queue size up to this moment.
...
public:
	Facility(std::string name, unsigned int numberOfServers, SimulatorGlobals &simulatorGlobals, Scheduler &scheduler);
	Facility(std::string name, SimulatorGlobals &simulatorGlobals, Scheduler &scheduler);
