...
TEST_F(ProtocolDataUnitTest, Routing) {
	...
	std::shared_ptr<ProtocolDataUnit> pdu(new ProtocolDataUnit(simulatorGlobals, 1, NULL, nodeVector.at(0), nodeVector.at(4), 1000));
	// Get initial TTL
	...
	EXPECT_FALSE(pdu->previous); // That's the way to test for NULL smart pointers.
